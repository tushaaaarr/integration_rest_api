...
def view_items(request):
    if request.query_params:
        items = TestModel.objects.filter(**request.query_param.dict())
    else:
        items = TestModel.objects.all()
  
    if items:
        data = TestApi_data(items,many =True)
...
def add_items(request):
    item = TestApi_data(data=request.data)
...
# def ItemsView(request):
    ...
    #     items = TestModel.objects.all()
    #     serializer = TestApi_data(items, many =True)
    #     return JsonResponse(serializer.data, safe =False)
 
    # elif request.method == 'POST':
    #     data = JSONParser().parse(request)
    #     serializer =TestApi_data(data = data)
